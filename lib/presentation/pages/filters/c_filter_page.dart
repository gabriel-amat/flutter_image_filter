...
final nativeLib =
    Platform.isAndroid
        ? DynamicLibrary.open("libnative_filter.so")
        : DynamicLibrary.process();

// Definição do tipo da função C
typedef GrayscaleFilterC = Void Function(Pointer<Uint8>, Int32, Int32);
typedef GrayscaleFilterDart = void Function(Pointer<Uint8>, int, int);

// Lookup da função
final grayscaleFilter = nativeLib
    .lookupFunction<GrayscaleFilterC, GrayscaleFilterDart>('apply_gray_filter');
...
class _CFilterPageState extends State<CFilterPage> {
  ...
  Future<void> _applyFilter() async {
    ...
    try {
      ...
      // Chama a função C
      debugPrint('Chamando função C...');
      grayscaleFilter(nativeBuffer, _width, _height);
      debugPrint('Função C executada!');
